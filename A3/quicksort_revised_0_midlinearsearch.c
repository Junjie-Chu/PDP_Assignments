...
int main(int argc, char **argv) {
    ...
    */
    ...
    double* finaldata;
    double max_parr_runtime;
    double max_loc_runtime;
    if (size > 1){
...
3.4 Merge the two sets of numbers in each process into one sorted list*/
    ...
    //No need to malloc high/low
    //double *low=(double *)malloc(chunk*sizeof(double));
    //double *high=(double *)malloc(chunk*sizeof(double));
    //Linear search, find the position of localpivot in the processor, starts from middle
    int lt,ht,middle,position;
    lt = 0;
    ht = 0;
    middle = chunk/2;
    position = 0;
    //find the position we want to insert the pivot, if pivot=data[i],we want the position to be i+1
    if (data[middle]>=localpivot){
      if(data[0]>localpivot){
	position = 0;
      }else{
     // printf("start the loop branch 1");
	...
	if (data[i]<=localpivot){
      ...
      if(data[chunk-1]<=localpivot){
        position = chunk;
      }else{
      for (int i = middle+1;i<chunk;i++){
	position = i;
	if (data[i]>localpivot){
	  //position = i;
	  //printf("\nposition:%d\n",i);
    ...
    //printf("\nfinal position:%d\n",position);
    // elements<pivot stored in low, elements>=pivot stored in high
    lt = position;
    ht = chunk - lt;
    int l,h; l=lt;h=ht;
/*
    //Linear search, find the position of localpivot in the processor
    int l,h;
    l = 0;
    h = 0;
    for (int i = 0; i < chunk; i++){
...
*/
    //int l,h; l=lt;h=ht;
    //printf("\nl:%d,h:%d,lt:%d,ht:%d\n",l,h,lt,ht);
    // l numbers(0 to l-1) < pivot, h numbers(l to l+h-1) > pivot.
    ...
    //No need to communicate the size
    /*
    //Communicate size
    ...
    if (rank < subsize){
    	...
    	//high data starts from data+l, there is h elements in high data.
    	MPI_Isend(data+l, h, MPI_DOUBLE, pairrankh, tag+1, group,&request1);
    	...
    	//low data starts from data, there is l elements in low data.
    	MPI_Isend(data, l, MPI_DOUBLE, pairrankl, tag, group,&request1);
    ...
    if (rank < subsize){
    	newdata = merge(data,l,otherlow,otherl);
	...
	for (int i=0; i<newdatasize; i++) {
        ...
        printf("\n");
        //merge do the sort so no need to use qsort!
	...
	printf("\nafter qsort\n");
 	...
 	return (parallel_quicksort(newgroup, newrank, newsize, newdata, newdatasize));
    	...
    	newdata = merge(data+l,h,otherhigh,otherh);
